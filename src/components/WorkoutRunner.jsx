...
function FeedbackForm({ exercises, onSubmit }) {
...
/**
 * The exercise checklist + prehab block + timers + feedback form, shared by
 * the anonymous /share/:token flow (AthleteView) and the authenticated
 * athlete portal's Today tab. `onSubmitFeedback` receives
 * { emoji_rating, rpe, notes, exercises_completed, exercises_total } and is
 * responsible for actually persisting it (with whatever workout_id /
 * athlete_id / completed_date the caller has available).
 */
export function WorkoutRunner({ workout, exercises, prehabExercises = [], onSubmitFeedback, onFeedbackSubmitted }) {
