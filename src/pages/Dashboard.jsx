      ...
      <div style={{ padding: '20px 24px' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
          ...
          <button onClick={() => navigate('/workout/new')}
            style={{ background: 'var(--ac)', color: '#0C1118', border: 'none', borderRadius: 'var(--r)', padding: '9px 16px', fontSize: 13, fontWeight: 700 }}>
            + New workout
          </button>
        </div>
        ...
        {workouts.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '60px 20px', color: 'var(--mu)' }}>
            ...
            <button onClick={() => navigate('/workout/new')}
              style={{ background: 'var(--ac)', color: '#0C1118', border: 'none', borderRadius: 'var(--r)', padding: '10px 20px', fontSize: 13, fontWeight: 700 }}>
              Create workout
            </button>
          </div>
        ) : (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: 14 }}>
            {workouts.map(w => (
              <div key={w.id} style={{ background: 'var(--s2)', border: '1px solid var(--br)', borderRadius: 12, padding: 16 }}>
                ...
                <div style={{ borderTop: '1px solid var(--br)', paddingTop: 12, display: 'flex', gap: 8 }}>
                  <button onClick={() => copyShareLink(w.share_token)}
                    style={{ flex: 1, background: 'transparent', border: '1px solid var(--br)', borderRadius: 6, color: copied === w.share_token ? 'var(--ac)' : 'var(--mu)', fontSize: 12, padding: '7px 10px' }}>
                    {copied === w.share_token ? '✓ Copied!' : '🔗 Share link'}
                  </button>
                  <button onClick={() => navigate(`/workout/${w.id}/edit`)}
                    style={{ background: 'transparent', border: '1px solid var(--br)', borderRadius: 6, color: 'var(--mu)', fontSize: 12, padding: '7px 10px' }}>
                    Edit
                  </button>
                  <button onClick={() => deleteWorkout(w.id)}
                    style={{ background: 'transparent', border: '1px solid var(--br)', borderRadius: 6, color: '#F88080', fontSize: 12, padding: '7px 10px' }}>
                    🗑
                  </button>
                </div>
              </div>
            ...
            <div onClick={() => navigate('/workout/new')}
              style={{ border: '1.5px dashed var(--br)', borderRadius: 12, padding: 16, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: 160, gap: 8, cursor: 'pointer', color: 'var(--mu)' }}>
